({
  onLoad() {
    ...
    function buildFakeMessage(chId, authId, content) {
      var author = UserStore ? UserStore.getUser(authId) : null;
    ...
    var cmds = [
      {
        ...
        execute: function(args, ctx) {
          try {
            ...
            var channel = ChannelStore ? ChannelStore.getChannel(channelId) : null;

            // Get current user
            var currentUser = CurrentUserStore ? CurrentUserStore.getCurrentUser() : null;
            var myId = currentUser ? currentUser.id : null;

            // Get the other person from DM recipients
            var targId = null;
            if (channel && channel.recipients && channel.recipients.length > 0) {
              targId = channel.recipients[0];
            }

            if (!myId || !targId) {
              showToast("Can't detect users. Use in a DM.", getAssetIDByName("Small"));
              return;
            }

            // Message 1: 3-5 hours earlier (looks like opened chat)
            var now = Date.now();
